...
struct TestParams
{
    ...
    uint32_t bufferSize;
    bool
        isCopySrcSparse; // true = source buffer in the copy command is sparse, false = destination buffer in the copy command is sparse
    bool isMultiCopy;
};
...
void BufferSparseResidencyNonResidentCase::copyVerificationProgram(SourceCollections &sourceCollections) const
{
    ...
    src << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
        ...
        << "{\n"
        << "    bool ok = true;\n"
        << "    for (uint offset = 0; offset < pc.bufferSize; offset += pc.blockSize)\n"
        << "    {\n";

    if (!m_testParams.isBufferNonResident)
    {
        src << "        uint idx = offset / pc.blockSize;\n"
            << "        if (idx % 2 == 0)\n"
            << "            ok = ok && (sbIn.data[offset] != 0);\n"
            << "        else\n";
    }

    src << "            ok = ok && (sbIn.data[offset] == 0);\n"
        << "    }\n"
        << "\n"
        << "    sbOut.result = ok;\n"
        << "}\n";
...
tcu::TestStatus BufferSparseResidencyNonResidentInstance::iterate(void)
{
    ...
    const VkDeviceSize outputTestBufferSizeBytes = sizeof(bool);
    ...
    if ((m_testParams.bufferInitCmd == BUFF_INIT_COPY) && !m_testParams.isCopySrcSparse)
    {
        ...
        const bool *outputData = static_cast<const bool *>(outputTestBufferAlloc->getHostPtr());
    ...
    else if ((m_testParams.bufferInitCmd == BUFF_INIT_COPY) && m_testParams.isMultiCopy)
    {
        ...
        for (uint32_t byteIdx = 0; byteIdx < 32u; byteIdx++)
        {
            ...
            uint32_t referenceValue = (nullMask & (1u << byteIdx)) ? 0xff : 0x00;
    ...
    if (!useDeviceGroups)
    {
        ...
        for (uint32_t cmdNdx = BUFF_INIT_COPY; cmdNdx < BUFF_INIT_LAST; cmdNdx++)
        {
            for (const auto bufferNonResidency : {true, false})
            {
                ...
                {
                    TestParams testParams = {static_cast<BufferInitCommand>(cmdNdx), bufferNonResidency, bufferSize,
                                             true /* isCopySrcSparse */, false /* isMultiCopy */};

                    const std::string testNameP1 =
                        std::string("non_resident_buffer") + "_" + getbufferInitCmdName(testParams.bufferInitCmd);
                    const std::string testNameP2 = std::string("_alloc_") + (bufferNonResidency ? "none" : "partial") +
                                                   "_" + de::toString(bufferSize);

                    const std::string testName = testNameP1 + ((cmdNdx == BUFF_INIT_COPY) ? "_src" : "") + testNameP2;
                    group->addChild(
                    ...
                    if (cmdNdx == BUFF_INIT_COPY)
                    {
                        ...
                        const std::string subtestName = testNameP1 + "_dest" + testNameP2;

                        group->addChild(
                            new BufferSparseResidencyNonResidentCase(group->getTestContext(), subtestName, testParams));
                    }
                }
            }
        }
        ...
        {
            TestParams testParams = {BufferInitCommand::BUFF_INIT_COPY, true /* completely non-resident */, 1 << 16,
                                     true /* isCopySrcSparse */, false /* isMultiCopy */};

            const std::string testName = std::string("non_resident_buffer") + "_multi_" +
                                         getbufferInitCmdName(BufferInitCommand::BUFF_INIT_COPY);
