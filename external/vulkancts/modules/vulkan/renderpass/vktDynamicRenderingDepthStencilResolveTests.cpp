...

{
    ...
    m_colorImage = de::MovePtr<ImageWithMemory>(new ImageWithMemory(m_vkd, m_device, allocator, colorImageParams, MemoryRequirement::Any));

    // Color image view
    m_colorAttachmentView = makeImageView(m_vkd, m_device, **m_colorImage, VK_IMAGE_VIEW_TYPE_2D, m_pcImageFormat, makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, 1u));
    ...
    // Output buffer
    const VkDeviceSize outputBufferSize = static_cast<VkDeviceSize>(
        static_cast<uint32_t>(tcu::getPixelSize(mapVkFormat(m_pcImageFormat))) * m_pcImageSize.x() * m_pcImageSize.y() * 1u);
    const auto outputBufferInfo = makeBufferCreateInfo(outputBufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT);

    m_outputColorBuffer = de::MovePtr<BufferWithMemory>(new BufferWithMemory(m_vkd, m_device, allocator, outputBufferInfo, MemoryRequirement::HostVisible));
}
    ...
    beginCommandBuffer(vkd, *cmdBuffer);
    {
        // Bind push constants and do DS resolve
        {
                                    ...
                                    0u, 0u, nullptr, 0u, nullptr, 1u, &preRenderBarrier);
            {
                ...
                vkd.cmdPushConstants(*cmdBuffer, *m_pcPipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0u, static_cast<uint32_t>(sizeof(pushConstants)), &pushConstants);
            }
        ...
        const VkImageMemoryBarrier outImageBarrier = makeImageMemoryBarrier(0u, VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, **m_colorImage, colorSubresourceRange);

        // Render for testing push constants
        {

            vkd.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, (VkDependencyFlags)0, 0, nullptr, 0, nullptr, 1, &outImageBarrier);
            ...
            vkd.cmdDraw(*cmdBuffer, 6u, 1u, 0u, 0u);
            vkd.cmdEndRendering(*cmdBuffer);
            copyImageToBuffer(vkd, *cmdBuffer, **m_colorImage, **m_outputColorBuffer, tcu::IVec2(m_pcImageSize.x(), m_pcImageSize.y()));
        }
    }
...
tcu::TestStatus DepthStencilPushConstResolveTest::iterate(void)
{
    ...
    const tcu::ConstPixelBufferAccess resultPixels{mapVkFormat(m_pcImageFormat), static_cast<int>(m_pcImageSize.x()), static_cast<int>(m_pcImageSize.y()), 1, resultsBufferPtr};
    ...
    result = tcu::floatThresholdCompare(m_context.getTestContext().getLog(), "ComparisonResult", "Image comparison result", referenceImage.getAccess(), resultPixels, tcu::Vec4(0.0f), tcu::COMPARE_LOG_ON_ERROR);
                 ...
                 ++separateDepthStencilLayouts)
            {
                     ...
                     depthResolveModeNdx++)
                {
                         ...
                         stencilResolveModeNdx++)
                    {
                        ...
                        if (hasDepth && !hasStencil && (dResolve.flag == VK_RESOLVE_MODE_AVERAGE_BIT) && (sResolve.flag == VK_RESOLVE_MODE_NONE)
